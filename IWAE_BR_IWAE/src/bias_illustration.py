...
if __name__ == "__main__":
    ...
    #bias_terms = ['constant', '1/4', '1/3', '1/2', '2/3', '3/4', '1']
    bias_terms = ['1/3']

    # Create a dictionary to store loss values for each model
    losses_dict = {bias: {'train': [], 'test': []} for bias in bias_terms}
    ...
    for bias in bias_terms:
        ...
        for epoch in range(params['num_epochs']):
            ...
            if bias == 'constant':
                ...
                b = int(np.ceil(np.power(epoch+1, 1/4)))
            elif bias == '1/3':
                b = int(np.ceil(np.power(epoch+1, 1/3)))
            elif bias == '1/2':
                b = int(np.ceil(np.sqrt(epoch+1)))
            elif bias == '2/3':
                b = int(np.ceil(np.power(epoch+1, 2/3)))
            elif bias == '3/4':
                b = int(np.ceil(np.power(epoch+1, 3/4)))
            elif bias == '1':
                b = epoch+1
            ...
            losses_dict[bias]['train'].append(avg_train_loss)
